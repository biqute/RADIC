/*
This code is based on the examples on the ALSA website and apparently it works.
...
static unsigned maxval = pow(2, 24) / 2 - 1;        // Maximum number obtainable with specified precision
...
// Tranfer method - write and wait for room in buffer using poll
...
static int write_and_poll_loop(snd_pcm_t *handle, signed long *samples, snd_pcm_channel_area_t *areas)
{
    struct pollfd *ufds;
    double phase = 0;
    signed long *ptr;
...
// Tranfer method - direct write only using mmap_write functions
static int direct_write_loop(snd_pcm_t *handle, signed long *samples, snd_pcm_channel_area_t *areas)
{
    double phase = 0;
    signed long *ptr;
    int err, cptr;
...
static struct transfer_method transfer_methods[] = 
{
    ...
    {"write and poll", SND_PCM_ACCESS_RW_INTERLEAVED, write_and_poll_loop},
    {"direct write", SND_PCM_ACCESS_MMAP_INTERLEAVED, direct_write_loop},
